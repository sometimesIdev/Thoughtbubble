//
//  ThoughtbubbleApiTests.swift
//  
//
//  Created by Pat Butler on 2022-03-05.
//

@testable import ThoughtbubbleApi
import XCTest

final class ThoughtBubbleApiTests: XCTestCase {
	...
	private func authenticateRoot() async throws -> User.Token.Detail {
		try await authenticate(.init(email: "root@localhost.com", password: "ChangeMe2"))
	}
	
	func testAuthorization() async throws {
		let login = User.Account.Login(email: "root@localhost.com", password: "ChangeMe2")
